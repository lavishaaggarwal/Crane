...
func (s *server) FetchFile(in *pb.HelloRequest, stream pb.Greeter_FetchFileServer) error {
	...
	output_files, err := exec.Command("ls", "/home/vandana2/425-MP3/sdfs/").Output()
     	if err != nil {
	...
	if strings.Contains(output, in.Name) {
		...
		file_read, err := ioutil.ReadFile("/home/vandana2/425-MP3/sdfs/"+in.Name)
...
func GetLiveNodes(new_info string) (pb.HelloReply) {
	fmt.Println("Inside GetLiveNodes", new_info)
	...
	for j := 0; j < len(neighbours); j++ {
		  ...
		  if strings.Compare(split_nbr[4], "A") == 0 {
			  ...
			  fmt.Println("File name", file_name[1])
  			  ...
  			  r, err := c.FetchFile(ctx, &pb.HelloRequest{Name: file_name[1]})
			  ...
			  file_output, err1 := r.Recv()
				...
				fmt.Println("GetLiveNodes output", file_output.Message)
				jj.Message = file_output.Message
				break
			  }
...
func (s *server) SayHello(in *pb.HelloRequest, stream pb.Greeter_SayHelloServer) error {
	...
	fmt.Println("Inside SayHello", in.Name)
	if strings.Contains(in.Name, "put") {
		fmt.Println("Fetch from live nodes")
